...
<!-- Workshop additions. Everything above is a swamp-managed block that
     `swamp repo init` rewrites; put local rules only below this line. -->

## Workshop rules

12. **Always pass `--json` to any `swamp ... search` command.** Without it, and
    with a terminal on stdin, `swamp model type search`, `swamp extension
    search`, `swamp model search`, and `swamp workflow search` all open a
    full-screen interactive fuzzy-picker TUI and block forever. A query argument
    does not avoid it — it only pre-fills the search box. This overrides the bare
    forms shown in rules 1 and 10 and in Getting Started above. Verified against
    swamp `20260823.004821.0`; see `FINDINGS.md` for the measurements.
